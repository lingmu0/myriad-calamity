...
public enum YangJianSkill {
    ...
    public int stepWindup(int step) { return windups[Math.clamp(step,0,windups.length-1)]; }
    public int stepStart(int step) {
        int time=0;
        for(int i=0;i<Math.clamp(step,0,steps());i++)time+=windups[i]+active+gap;
        return time;
    }
    ...
    public static int restoredPhase(int saved) { return Math.clamp(saved,1,3); }
    ...
    public static int consumeAxeTrailCharge(int charges) {
        int bounded=Math.clamp(charges,0,AXE_TRAIL_CHARGES);
