...
public final class CogworkBlade extends Projectile {
    ...
    @Override protected void defineSynchedData() { this.entityData.define(SILVER,false); }
    ...
    public static void spawnWave(ServerLevel level,CogworkDancer owner,Vec3 center,int count,float speed,float damage,double angleOffset) {
        ...
        int number=Math.min(Mth.clamp(count,0,MAX_PER_WAVE),Math.max(0,MAX_PER_OWNER-existing));
        if(number==0)return;
        float velocity=Mth.clamp(speed,.1F,1.2F);
        for(int i=0;i<number;i++) {
            ...
            blade.setOwner(owner);blade.origin=center;blade.damage=Mth.clamp(damage,0.1F,1000F);
    ...
    @Override protected void readAdditionalSaveData(CompoundTag tag) {
        super.readAdditionalSaveData(tag);life=Mth.clamp(tag.getInt("BladeLife"),0,MAX_LIFE);damage=Mth.clamp(tag.getFloat("BladeDamage"),0.1F,1000F);
