...
public final class YangJianHazard extends Entity {
    ...
    @Override protected void defineSynchedData() { this.entityData.define(PLAN,new CompoundTag()); }
    ...
    @Nullable public static YangJianHazard beam(ServerLevel level,YangJian owner,Vec3 origin,Vec3 direction,double range,double radius,int warning,int active,int mode,float damage) {
        ...
        h.bind(owner,damage);CompoundTag p=h.base(1,warning,Mth.clamp(active,1,100),radius);
        // Mode 3 is a short clone-sweep pass; it shares the beam renderer with
        // the normal sweep but turns farther each active tick.
        p.putInt("mode",Mth.clamp(mode,0,4));p.putDouble("range",Mth.clamp(range,1,48));
    ...
    private void bind(YangJian owner,float amount) {
        caster=owner.getUUID();lease=owner.attackSequence();damage=Float.isFinite(amount)?Mth.clamp(amount,.1F,10000):1;
    ...
    private CompoundTag base(int kind,int warning,int active,double radius) {
        ...
        p.putInt("w",Mth.clamp(warning,minimum,80));p.putInt("active",active);p.putFloat("radius",(float)YangJianHazardMath.impactRadius(kind,radius));return p;
    }
    ...
    public Vec3 visualDirection(float partial) {
        ...
        if(mode()==2 && active(partial))return read(p,"previous").lerp(current,Mth.clamp(partial,0,1)).normalize();
        return current;
    }
    ...
    @Override public void tick() {
        ...
        if(kind()==1) {
            ...
            if(!impacted) {
                impacted=true;
                level().playSound(null,blockPosition(),SoundEvents.TRIDENT_THUNDER,SoundSource.HOSTILE,1,1.35F);
            }
            damageGroundTargets(server,owner);
        } else if(!impacted) {
            impacted=true;
            level().playSound(null,blockPosition(),kind()==3?SoundEvents.TRIDENT_THUNDER:SoundEvents.TRIDENT_HIT_GROUND,SoundSource.HOSTILE,1,kind()==3?1.35F:.8F);
            damageGroundTargets(server,owner);
        }
    }
