...
public final class YangJianEffects {
    ...
    public static int swordCount(int requested) { return Mth.clamp(requested,0,MAX_SWORDS); }
    ...
    public static int myriadReleaseTick(int index) { return MYRIAD_ORBIT_TICKS+Mth.clamp(index,0,MYRIAD_SWORDS_PER_WAVE-1); }
    ...
    public static Direction myriadOrbit(double age,int wave,int index) {
        int slot=Mth.clamp(index,0,MYRIAD_SWORDS_PER_WAVE-1);
    ...
    public static boolean groundContact(double px,double py,double pz,double sx,double sy,double sz,
        ...
        double t=lengthSquared<1E-9?0:Mth.clamp(((px-sx)*dx+(pz-sz)*dz)/lengthSquared,0,1);
