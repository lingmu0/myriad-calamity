...
public final class YangJianFootwork {
    ...
    public static double distance(double targetDistance,int phase) {
        return Double.isFinite(targetDistance)?Math.clamp(targetDistance-stoppingDistance(phase),0,MAX_DISTANCE):0;
    }
    public static double progress(double age) {
        double t=Math.clamp((age-WINDUP)/(MOVE_END-WINDUP),0,1);
