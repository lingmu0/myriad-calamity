...
public final class LightningTrailRenderer extends EntityRenderer<LightningTrail> {
    ...
    private static void vertex(VertexConsumer v,PoseStack pose,Vec3 p,int color) {
        v.vertex(pose.last().pose(),(float)p.x,(float)p.y,(float)p.z).color(color).endVertex();
    }
    private static int color(double alpha,int rgb) { return ((int)Mth.clamp(alpha,0,255)<<24)|rgb; }
