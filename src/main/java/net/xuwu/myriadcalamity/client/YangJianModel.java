...
public final class YangJianModel extends EntityModel<YangJian> {
    ...
    private void aimHead(float bodyYaw,Vec3 direction) {
        ...
        Bone head=headPath.get(headPath.size()-1);
    ...
    public void renderBeamClone(PoseStack pose,VertexConsumer buffer,int light,int overlay,int color,
        ...
        try {
            ...
            render(pose,buffer,light,overlay,color);
    ...
    private boolean findWeaponPath(Bone bone) {
        ...
        weaponPath.remove(weaponPath.size()-1);return false;
    }
    ...
    private void renderBone(Bone b,PoseStack pose,VertexConsumer buffer,int light,int overlay,int color,boolean isolatedWeapon) {
        ...
        if(!isolatedWeapon && b.name.equals("weapon") && selectedWeapon>0) {
            ...
            for(Face face:b.faces)for(float[] v:face.vertices())
                buffer.vertex(pose.last().pose(),v[0]/16F,v[1]/16F,v[2]/16F).color(color).uv(v[3],v[4])
                    .overlayCoords(overlay).uv2(face.glow()?15728880:light)
                    .normal(pose.last().normal(),face.normal()[0],face.normal()[1],face.normal()[2]).endVertex();
        }
        if(b.name.equals("head") && eyeGlow>0 && !isolatedWeapon) {
            ...
            for(int i=0;i<4;i++)buffer.vertex(pose.last().pose(),corners[i][0]/16F,corners[i][1]/16F,z/16F)
                .color(0xFFFFEC9C).uv((i==0 || i==3?1127F:1129F)/2048F,(i<2?594F:596F)/2048F)
                .overlayCoords(overlay).uv2(15728880).normal(pose.last().normal(),0,0,-1).endVertex();
        }
